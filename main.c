...
static image_t kpu_image, kpu_od_image, display_image;//42

kpu_model_context_t lp_detect_task;
kpu_model_context_t recog_task;
kpu_model_context_t obj_detect_task;//44
...
static void lable_init(void)//203
{
	uint8_t index;
...
static void drawboxes(uint32_t x1, uint32_t y1, uint32_t x2, uint32_t y2, uint32_t class, float prob)//219
{
	if (x1 >= 320)
		x1 = 319;
	if (x2 >= 320)
...
static void draw_edge(uint32_t *gram, obj_info_t *obj_info, uint32_t index, uint16_t color)//234
{
...
int main(void)
{
    ...
    if (kpu_load_kmodel(&obj_detect_task, detect_data) != 0)//365
    {
        printf("\nmodel init error\n");
        while (1);
    }
    ...
    obj_detect_rl.anchor_number = ANCHOR_NUM;//370
    obj_detect_rl.anchor = anchor1;
